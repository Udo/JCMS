 ...
 * @version $Id: PreparedStatement.java,v 1.27.2.14 2003/05/22 18:49:16 mmatthew Exp $
 *
...
public class PreparedStatement extends com.mysql.jdbc.Statement
    ...
    /** Does this statement require the retrieval of generated keys? */
    protected boolean retrieveGeneratedKeys = false;
    private java.sql.DatabaseMetaData dbmd = null;
    private SimpleDateFormat timestampDateFormat = null;
    ...
    public PreparedStatement(Connection conn, String sql, String catalog)
        ...
        for (int i = 0; i < statementLength; i++) {
            ...
            if ((this.firstCharOfStmt == 0)
                    && !Character.isWhitespace(statementAsChars[i])) {
        ...
        for (i = 0; i < staticSqlStrings.length; i++) {
            if (this.isLoadDataQuery) {
                ...
                staticSqlStrings[i] = temp.getBytes();
    ...
    public void setBytes(int parameterIndex, byte[] x)
        ...
        if (x == null) {
            ...
            if (this.connection.useUnicode() && (encoding != null)
                    && ("Shift_JIS".equals(encoding) || "SJIS".equals(encoding))) {
                bOut.write('x');
                bOut.write('\'');
                ...
                for (int i = 0; i < numBytes; i++) {
                    ...
                    int highNibble = hexVal / 16;
                    int lowNibble = hexVal % 16;

                    bOut.write(HEX_BYTES[highNibble]);
                    bOut.write(HEX_BYTES[lowNibble]);
                }
    ...
    /**
     * Sets whether this statement should setup the result set to be able to
    ...
    private void setTimestampInternal(int parameterIndex, Timestamp x,
        ...
        if (x == null) {
            ...
            synchronized (getTimestampDateFormat()) {
                timestampString = "'" + getTimestampDateFormat().format(x)
                    + "'";
            }
    ...
    private final void streamToBytes(Buffer packet, InputStream in,
        ...
        if (this.connection.useUnicode() && (encoding != null)
                && ("Shift_JIS".equals(encoding) || "SJIS".equals(encoding))) {
            escapeStreamHex(packet, in, escape, streamLength, useLength);
        } else {
            escapeStreamSlashes(packet, in, escape, streamLength, useLength);
        }
    }

    private void escapeStreamHex(Buffer packet, InputStream in, boolean escape,
        ...
        try {
            ...
            while (bc > 0) {
                if (escape) {
                    for (int i = 0; i < bc; i++) {
                        int hexVal = this.streamConvertBuf[i] & 0xFF;

                        int highNibble = hexVal / 16;
                        int lowNibble = hexVal % 16;

                        packet.writeByte(HEX_BYTES[highNibble]);
                        packet.writeByte(HEX_BYTES[lowNibble]);
                    }
                } else {
    ...
    private void escapeStreamSlashes(Buffer packet, InputStream in,
        boolean escape, int streamLength, boolean useLength)
        throws SQLException {
        try {
            int bc = -1;
    ...
    private synchronized SimpleDateFormat getTimestampDateFormat() {
        if (this.timestampDateFormat == null) {
            this.timestampDateFormat = new SimpleDateFormat(
                    "yyyy-MM-dd HH:mm:ss");
        ...
        return this.timestampDateFormat;
    }
