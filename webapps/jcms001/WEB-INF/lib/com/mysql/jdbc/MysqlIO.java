 ...
 * @version $Id: MysqlIO.java,v 1.32.2.26 2003/05/19 14:43:06 mmatthew Exp $
 */
public class MysqlIO {
	...
	/**
	 * Does the character set of this connection match the
     ...
     */
    void doHandshake(String user, String password) throws java.sql.SQLException {
        String seed;
                ...
                if (this.protocolVersion > 9) {
                    ...
                    if (xOpen.equals("S1000")) {
                        ...
                        if ((message != null) && (message.length() > 0)) {
                            mesg.append(" Message from server: \"");
                            mesg.append(message);
                            mesg.append("\"");
        ...
        if (rowPacket.readByte() == (byte) 0xff) {
            ...
            if (this.protocolVersion > 9) {
                ...
                throw new java.sql.SQLException(errorMessage,
                    SQLError.mysqlToXOpen(errno), errno);
            }
        }
     ...
     */
    final void quit() throws IOException, SQLException {
    ...
    final ResultSet sqlQueryDirect(Buffer queryPacket, int maxRows,
        ...
        if (columnCount == 0) {
            ...
            if (this.platformDbCharsetMatches) {
            	...
            	? resultPacket.readString(charEncoding) : resultPacket.readString());
            } else {
	...
	protected final Field unpackField(Buffer packet,
		...
		if (this.use41Extensions) {
			...
			int colLength = packet.readLongInt();
    ...
    private final Buffer reuseAndReadPacket(Buffer reuse)
        ...
        if (packetLength == maxThreeBytes) {
            ...
            while (true) {
                ...
                if ((packetLength == 1)
                        || (this.useNewLargePackets && (packetLength == 0))) {
                ...
                if (bytesRead != lengthToWrite) {
                    throw new SQLException("Short read from server, expected "
                        + lengthToWrite + " bytes, received only " + bytesRead
                        + ".", "08S01");
                }
     ...
     */
    private final void send(Buffer packet) throws IOException, SQLException {
        ...
        if ((serverMajorVersion >= 4) && (packetLen >= maxThreeBytes)) {
            ...
            if (useCompression) {
                ...
                if (packetLen < MIN_COMPRESS_LEN) {
                    ...
                    Buffer compressedPacket = new Buffer(packetLen
                            + headerLength);
                    ...
                    int compLen = deflater.deflate(compressedBytes,
                            headerLength, packetLen);
                    compressedPacket.setPosition(0);
    ...
    private final ResultSet sendFileToServer(String fileName)
        ...
        int packetLength = Math.min(this.connection.getMaxAllowedPacket() - (HEADER_LENGTH * 3), alignPacketSize(this.connection.getMaxAllowedPacket()
                - 16, 4096) - (HEADER_LENGTH * 3));
    ...
    private Buffer checkErrorPacket(int command)
        ...
        try {
            ...
            throw new java.sql.SQLException(SQLError.get("08S01") + ": "
                + fallThru.getClass().getName(), "08S01", 0);
        }
    ...
    private final void sendSplitPackets(Buffer packet)
        ...
        if (Driver.DEBUG) {
            System.out.println("\n\nSending split packets for packet of " + len
                + " bytes:\n");
        }
     ...
     * @throws IOException
     */
    private void secureAuth(int packLength, long clientParam, String user,
	...
	/**
	 * Determines if the database charset is the same
	 * as the platform charset
	 */
	protected void checkForCharsetMismatch() {
		if (this.connection.useUnicode() && this.connection.getEncoding() != null) {
			this.platformDbCharsetMatches = 
				System.getProperty("file.encoding").equals(this.connection.getEncoding());
		}
	}
}
